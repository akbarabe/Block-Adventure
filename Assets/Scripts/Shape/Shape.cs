...
public class Shape : MonoBehaviour, IPointerClickHandler, IPointerUpHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerDownHandler
{
        ...
        for (var row = 0; row < shapeData.rows; row++)
        {
            for (var column = 0; column < shapeData.columns; column++)
            {
                if (shapeData.board[row].column[column])
                {
                    ...
                    _currentShape[currentIndexInList].GetComponent<RectTransform>().localPosition = new Vector2(GetXPostionForShapeSquare(shapeData, column, moveDistance), GetYPositionForShapeSquare(shapeData, row, moveDistance));
                    currentIndexInList++;
                }
            }
        }
    ...
    private float GetYPositionForShapeSquare(ShapeData shapeData, int row, Vector2 moveDistance)
    {
        ...
        if (shapeData.rows > 1)
        {
            ...
            else
            {
                ...
                var middleSquareIndex1 = (shapeData.rows == 2) ? 0 : shapeData.rows - 2; // Mungkin salah karena dia bilangnya - 1, tapi nulisanya - 2
    ...
    private float GetXPostionForShapeSquare(ShapeData shapeData, int column, Vector2 moveDistance)
    {
        float shiftOnX = 0f;
        ...
        if (shapeData.columns > 1) //Vertical position calculation
        {
            if (shapeData.columns % 2 != 0)
            {
                ...
                if (column < middleSquareIndex) // move it on the negative postion
                {
                    shiftOnX = moveDistance.x * -1;
                    shiftOnX *= multiplier;
                }
                else if (column > middleSquareIndex) //move it on the plus 
                {
                    shiftOnX = moveDistance.x * 1;
                    shiftOnX *= multiplier;
                }
