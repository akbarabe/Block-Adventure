...
public class Scores : MonoBehaviour
{
    ...
    void Start()
    {
        currentScores_ = 0;
        newBestScore_ = false;
        squareTextureDat.SetStartColor();
        UpdateScoreText();
    }
    ...
    {
        if (GameEvents.UpdateSquareColor != null && currentScores_ >= squareTextureDat.tresholdVal)
        {
            squareTextureDat.UpdateColors(currentScores_);
            GameEvents.UpdateSquareColor(squareTextureDat.currentColor);
        }
    }
